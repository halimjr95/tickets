    ...
    <div class="mb-4 flex justify-between">
        ...
        <div class="flex space-x-2">
            ...
            <select
                ...
                <option value="{{ toggle('priority', $priority->value) }}" @selected($priority->value ==
                    request('priority'))>{{ $priority->name }}</option>
                @endforeach
            </select>
            ...
            <select
                ...
                <option value="{{ toggle('category', (string) $id) }}" @selected($id==request('category'))>{{ $name }}
                </option>
                @endforeach
            </select>
        </div>
                ...
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tbody class="bg-white divide-y">
                        ...
                        <tr class="text-gray-700">
                            ...
                            <td class="px-4 py-3 text-sm">
                                <a href="{{ route('tickets.show', $ticket) }}" class="hover:underline">{{ $ticket->title
                                    }}</a>
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{ $ticket->user->name }}
                            ...
                            <td class="px-4 py-3 text-sm">
                                ...
                                <span class="rounded-full bg-red-50 px-2 py-2">{{ $ticket->status }}</span>
                                @elseif ($ticket->status =='closed')
                                <span class="rounded-full bg-blue-50  px-2 py-2">{{ $ticket->status }}</span>
                                @elseif ($ticket->status =='archived')
                                <span class="rounded-full bg-green-50  px-2 py-2">{{ $ticket->status }}</span>
                                @endif
